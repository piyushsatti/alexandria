...
def verify_prepared(root):
    ...
    for row, expected in zip(manifest["files"], plan["files"], strict=True):
    ...
    assets = asset_root()
    skill, provenance = prep._skill(assets)
    if (
...
def execute_prepared(root, *, process_runner=run_process):
    ...
    assets = asset_root()
    node_modules = node_modules_root(assets)
    ...
    package_identities = {
        name: prep.digest(graph.read_bytes(PACKAGE_DIR / name, 2_000_000))
        for name in (
            "operator_run.py",
            "preparation.py",
    ...
    asset_identities = {
        name: prep.digest(graph.read_bytes(assets / name, 2_000_000))
        for name in (
    ...
    identities = {**package_identities, **asset_identities}
    write_receipt(
        root,
        "operator-start.json",
        {
    ...
    try:
        code = process_runner(
            [node, str(assets / "pi-trial/preservation-trial.mjs"), str(root)],
            config,
        )
