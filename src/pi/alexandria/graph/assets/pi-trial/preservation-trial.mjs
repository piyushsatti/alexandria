...
const schema=`Return {coverage:[{id,disposition,claim_ids,annotation_ids,reason}],claims:[{id,subject,predicate,object,path,quote,source_status,review_state,scope:{text,path,quote},annotation_ids:[]}],annotations:[{id,path,quote,kind,reason,resolution_question}],pages:[{title,sections:[{heading,paragraphs:[{text,claim_ids,annotation_ids}]}]}]}. Use only supplied paths and exact contiguous source quotes, each uniquely locatable. source_status must be accepted, proposed, open, requirement, limitation, historical, or deferred. review_state must be not_checked or held; use held for ambiguity, scope tension, unresolved interpretation, or review-state conflicts. Annotation kind must be ambiguity, open_decision, scope_tension, or interpretation.
