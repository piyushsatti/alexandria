...
from pi.alexandria.graph.graph import no_symlinks, read_bytes
from pi.alexandria.graph.harness import git, snapshot
...
def _skill(root=None):
    skill_root = (Path(root) if root is not None else asset_root()) / "fixtures"
...
def preflight(repo, revision, paths, model, approved_paths, output):
    ...
    _skill(assets)
    required = [
        assets / "pi-trial/preservation-trial.mjs",
        assets / "pi-trial/preservation.mjs",
        PACKAGE_DIR / "render_preservation.py",
        PACKAGE_DIR / "graph.py",
        assets / "pi-trial/package-lock.json",
    ]
