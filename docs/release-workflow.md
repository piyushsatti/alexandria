...
the selected revision, an active index built from another revision, or an active
...
2. Run `pi.alexandria.runtime.prepare_data_block` to copy the selected generation,
   source snapshot, model files, and receipt into a new data block.
3. Build `docker/Dockerfile.release` for the target architecture. Pass the
   application revision, indexed revision, CalVer release, and model as image
   labels.
4. Start a fresh container with `/data` mounted read-only.
5. Verify `status`, semantic search, literal search, document reads, path
   rejection, and container replacement persistence.
6. Record the image digest, data-block receipt, source hashes, model identity,
