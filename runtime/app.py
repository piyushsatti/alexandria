...
# The runtime is intentionally usable as a single-file stdio entrypoint as
# well as a package installed in the image.
if str(Path(__file__).resolve().parent) not in sys.path:
    sys.path.insert(0, str(Path(__file__).resolve().parent))
from listing import committed_tree, inbound_tree, validate_relative_path
...
def serve(
    ...
    *,
    ...
    except ImportError:  # pragma: no cover - only old SDKs lack Context
        Context = object
    ...
    count_tokens = token_counter(engine)
    from graph_access import OptionalGraph
    from inbound import InboundStore, authenticated_identity
    ...
    if transport == "streamable-http":
        from auth import LogtoJwtVerifier
    ...
    def search(
        ...
        if layer in ("committed", "all"):
            ...
            for row in rows:
                ...
                item["source"] = {
                    "repository": "https://github.com/piyushsatti/knowledge",
    ...
    def read_document(
        ...
        from listing import frontmatter
