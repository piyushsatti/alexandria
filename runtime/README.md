...
```text
python runtime/app.py serve --data /path/to/data
```
...
Use `runtime/prepare_data_block.py` with an exact Knowledge checkout, exact
...
hash before publishing the receipt. Build the image from this directory’s
`runtime/` folder; the image does not copy the output block.
