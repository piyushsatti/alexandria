...
```text
PYTHONPATH=graph:runtime python3 -m unittest discover -s graph/tests -p 'test_*.py'
node --test graph/tests/test_preservation*.mjs
```

The `graph/pi-trial` scripts are the narrow preservation contract used by the
offline tests. They are not part of the runtime image and do not choose a
provider, read credentials, or publish a graph automatically.
