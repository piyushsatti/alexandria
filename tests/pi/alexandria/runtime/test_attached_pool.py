...
from pi.alexandria.runtime.inbound import MAX_CONTENT_BYTES, InboundStore
from pi.alexandria.runtime.listing import committed_tree, frontmatter
...
class AttachedPoolTests(unittest.TestCase):
    ...
    def test_partial_index_is_repaired_and_requeues_indexed_content(self):
        submitted = self.inbound.submit(
            content="durable inbound content\n",
            session_label="s",
            idempotency_key="partial-index",
        )
        with self.inbound._connection() as connection:
            connection.execute(
                "UPDATE submissions SET status='indexed' WHERE id = ?",
        ...
        fake_lancedb = SimpleNamespace(connect=lambda _path: database)
        with patch.dict(sys.modules, {"lancedb": fake_lancedb}):
            _database, passages, documents = self.inbound._open_tables()

        self.assertIsNone(passages)
        self.assertIsNone(documents)
        self.assertEqual(database.dropped, ["passages"])
        receipt = self.inbound.receipt(submitted["submission_id"])
        self.assertEqual(receipt["status"], "queued")
        self.assertEqual(receipt["attempts"], 0)
        self.assertEqual(receipt["error"], "inbound_index_repair")
